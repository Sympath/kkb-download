...
(async function () {
  ...
  try {
    ...
    function writeConfig(index, course_id, course_name) {
      let {
        Authorization,
      } = commonInfo;
      // 保存下课程信息
    ...
    `
      let configPath = path.resolve(__dirname, `../${configDir}/config${index}.js`);
    ...
    intercept(page, patterns.XHR('*list?type=1&option=2*'), {
      ...
      onResponseReceived: event => {
        ...
        // 4. 收集信息 course_id 课程名称 Authorization cookie
    ...
    if (courseList.length > 0) {
      for (let index = 0; index < courseList.length; index++) {
        ...
        // 如果是全部课程下载
        if (noNeedFilter) {
          writeConfig(index, course_id, course_name)
        } else {
          // 如果是指定课程才下载 
