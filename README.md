...
2. 小节维度的去缓存：当小节下载成功后，将对应记录命令的 txt 文件改名为 -cache.txt；再一次下载时不会再读取到这个文件
3. 课程级别的去缓存：
   1. 当课程下载完成后会删除 output 中的课程资源，而读取命令的 getFileArr_1.js 中如果文件不存在则不进行读取记录；目前改为直接读取 output 中的目录，从而做到只要【上传完就删，一定不会重复读取】
   2. 如果有课程漏下载了，支持传递指定课程 id，从而做到指定课程的下载
